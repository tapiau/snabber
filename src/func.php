...
function printr($tab,$return = false)
{
	$dbg = debug_backtrace();
...
function printrlog($tab)
{
	$dbg = debug_backtrace();
	$msg = str_repeat('#',120)."\n";
	$msg .= now()." {$dbg[0]['file']}:{$dbg[0]['line']}\n";
	$msg .= str_repeat('#',120)."\n";
	$msg .= print_r($tab,true);
	$msg .= "\n\n";
	
	file_put_contents('/tmp/zibilog',$msg,FILE_APPEND);
}
