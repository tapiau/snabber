...
class Collection extends \ArrayObject
{
    ...
    public function groupBy($key=null)
    {
		...
		{
			return new self(array_reduce($array, function ($ret, $item) use ($key) {
				if (array_key_exists($item[$key], $ret)) {
					$ret[$item[$key]][] = $item;
				} else {
					$ret[$item[$key]] = new self([$item]);
				}
	...
	{
		$keyList = array_slice($this->keys()->getArrayCopy(),$offset,$length);
    ...
    public function merge($array = null)
    {
        if (is_null($array)) {
            ...
            return new self(array_merge($this->getArrayCopy(), $array));
        }
    }
